...
const TreeChopGame = ({ handleEndGame
}: {
    handleEndGame: () => void
}) => {
    ...
    const [apiLoading, setApiLoading] = useState<{ loading: boolean; to: string }>({
        loading: false,
        to: ''
    })
    ...
    const handleContinueGame = async () => {
        ...
        if (numOfContinues.current > 10) { //Fixpp Maybe needed for numOfContniues
    ...
    //Retry the game - set the score to 0
    ...
    const gameOver = async () => {
        try {
            ...
            await endGameSession({}, {
                onSuccess: (data) => {
                    sessionEndStamp.current = Date.now();
                    setSessionEndData({
        ...
        <>
            ...
            <div className="relative w-full h-screen overflow-hidden bg-sky-500">
                ...
                {showGameOver && <GameOverScreen
                    ...
                    score={sessionEndData.score}
                    chi={sessionEndData.chi}
